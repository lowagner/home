...
int highest_land(float x, float z) {
    // TODO:  switch over to foot_collide for the actual height to put player at
    int result = -1;
    int nx = roundf(x);
    int nz = roundf(z);
    int p = chunked(x);
...
static int count_item_faces(int fnx, int fpx, int fpy, int fny, int fnz, int fpz, W w) {
    switch (w.shape) {
        case S_CUBE:
        ...
        case S_STAIR_PXPZ:
            return 1 + fpx + 1 + fny + 1 + fpz;
        case S_STAIR_NXPZ:
            return fnx + 1 + 1 + fny + 1 + fpz;
        case S_STAIR_NXNZ:
            return fnx + 1 + 1 + fny + fnz + 1;
        case S_STAIR_PXNZ:
            return 1 + fpx + 1 + fny + fnz + 1;
        case S_HALF_PX:
            return 1 + fpx + fpy + fny + fnz + fpz;
        case S_HALF_NX:
            return fnx + 1 + fpy + fny + fnz + fpz;
        case S_HALF_PZ:
...
static int add_item_faces(float *data, float ao[6][4], float light[6][4],
...
void init_chunk(Chunk *chunk, int p, int q) {
    ...
    int dx = p * CHUNK_SIZE - CHUNK_PAD;
    int dy = 0;
    int dz = q * CHUNK_SIZE - CHUNK_PAD;
...
void set_block(int x, int y, int z, W w) {
    int p = chunked(x);
    int q = chunked(z);
    _set_block(p, q, x, y, z, w.value, 1);
    W nw = (W){.shape=-w.shape, .material=w.material, .color=w.color, .action=w.action};
    for (int dx = -1; dx <= 1; dx++) {
        for (int dz = -1; dz <= 1; dz++) {
            if (dx == 0 && dz == 0) {
                continue;
            ...
            _set_block(p + dx, q + dz, x, y, z, nw.value, 1);
        }
    }
    client_block(x, y, z, w.value);
}

void record_block(int x, int y, int z, W w) {
...
void builder_block(int x, int y, int z, W w) {
    if (y <= 0 || y >= 256) {
        return;
    }
    if (is_destructable(get_block(x, y, z))) {
        set_block(x, y, z, (W){.value=0});
    }
    if (w.shape) {
        set_block(x, y, z, w);
    }
...
void paste() {
    ...
    for (int y = 0; y < 256; y++) {
        for (int x = 0; x <= dx; x++) {
            for (int z = 0; z <= dz; z++) {
                W w = get_block(c1->x + x * scx, y, c1->z + z * scz);
...
void tree(Block *block) {
    ...
    for (int y = by; y < by + 7; y++) {
        builder_block(bx, y, bz,
            (W) {.shape=S_CUBE, .material=M_WOOD, .color=0, .action=0});
    }
}
...
void on_remove_block() {
    ...
    if (hy > 0 && hy < 256 && is_destructable(hw)) {
        ...
        if (is_plant(get_block(hx, hy + 1, hz))) {
            set_block(hx, hy + 1, hz, (W){.value=0});
        }
    }
}
...
void init_M() {
    ...
    g->M_index = 1;
    int index = 0;
    g->M[index  ][0] = (W){.shape=S_CUBE, .material=M_CLOUD, .color=127, .action=A_CLOUD};
